...
- **E2E는 3~5개, 이름을 정해둔다**: 진입(가입/로그인) · 핵심 동작 · 결제(돈이 움직이면) · 파괴적/설정 플로우 1개. 회귀가 새어나갔을 때만 추가. 원인을 고친 뒤에도 2번 flaky하면 **삭제하지 않고** 같은 시나리오를 `browser`/`unit` 계층으로 내린다(시나리오는 유지). 고정 시간 대기 금지.
