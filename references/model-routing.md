...
**원칙**: 본 세션(오케스트레이터)은 **opus**로 띄우고(adr/0011 개정 1 — 2026-08-21 실측에서 본체가 비용의 78%), 서브에이전트는 역할별로 명시한다. Fable은 사람이 `/model fable`로 올리는 자리(아래 표 "본체↑")에서만. 추론·판단·리서치가 필요한 자리만 본체가 직접 하고, 나머지는 Agent/Workflow 호출에 `model:`을 명시한다. 비용 측정은 `reports/`의 "소요" 줄로(2026-08-21 Tier-2 한 번 $7.20 중 리뷰 서브에이전트 58k 토큰이 기준선).

## 1. 라우팅 표
| 작업 | 모델 | 근거 |
|---|---|---|
| 티어 판정, Tier-2 질문 설계, 계약 상태 판정(A/B/C/D), 스펙 diff 제안, 구현·테스트 작성·증거 수집(위임 안 할 때) | **본체 = opus** | Tier-2 런 2회에서 본체가 78% — 이 일들은 opus 급. 티어 판정 오류가 2회 나오면 재검토 |
| Tier-3 `ce-brainstorm`·change 분해, 디자인 갭 후보 2~3안 판단, 새 방법론·도구 평가, 플러그인 자체 개선 | **본체↑ = fable** (사람이 `/model fable`) | 불확실성이 큰 자리만 |
