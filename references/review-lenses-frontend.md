...
## 6. persona 토큰 예산 (개정 2)
모든 persona는 **diff와 증거 묶음(review-fe 3단계)으로 시작**하고, 판정에 필요한 파일만 연다. `openapi.yaml`·ADR·`tokens.css`·스크린샷 전부를 통째로 읽지 않는다(2026-08-21 L6 87k·L4 88k). 목표 렌즈당 ≤50k. 넘으면 review-fe가 보고에 적는다.

## 5. 재검토 조건
- 어떤 렌즈가 3개 change 연속 "없음" → 그 렌즈는 Tier-3에서만
- 두 렌즈가 같은 지적을 반복 → 합친다
- 렌즈 지적이 린트로 잡을 수 있는 것이었음 2회 → 린트 규칙으로 승격하고 렌즈에서 뺀다
- 리뷰 비용이 구현 비용을 넘는 change 2개 → Tier-2 기본 렌즈를 L1 L2 L3로 줄이고 나머지는 UI/API 조건부 유지 — **2026-08-21 3회 충족 → 개정 2 적용**: `/ce-simplify-code`는 Tier-2 선택 항목, 2차 재실행은 blocker 렌즈만, persona는 diff+증거 묶음으로 시작하고 필요한 파일만 연다(§6). 렌즈 수·증거 상태 수는 유지(L4·L5가 실제 결함을 잡는 원천)
