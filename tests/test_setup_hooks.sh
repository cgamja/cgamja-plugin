...
check "bash: gen.ts sed deny"             "생성물" "$(hook $B "$(bashcmd 'sed -i s/a/b/ src/domains/todos/todos.gen.ts')")"
...
check "files: gen.ts deny"                "생성물" "$(hook $F "$(filecmd src/domains/todos/todos.gen.ts)")"
...
check "bash: gen.ts read with 2>&1 allow" "" "$(hook $B "$(bashcmd 'grep -n foo src/domains/todos/todos.gen.ts 2>&1')")"
check "bash: gen.ts redirect still deny" "deny" "$(hook $B "$(bashcmd 'echo x > src/domains/todos/todos.msw.ts')")"
