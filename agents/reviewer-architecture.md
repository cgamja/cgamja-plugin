...
2. **계약**: 생성물(`contract.generated`) 밖의 직접 HTTP 호출/손으로 쓴 응답 타입·스키마 → blocker. 코드·경계 mock 핸들러·테스트 픽스처에 계약 원천(`contract.source`)에 없는 필드/엔드포인트가 있으면 blocker("상태 D — 스펙 diff 먼저", `references/api-contract.md`). 재생성 후 diff 0 증거가 있는가. `contract`가 null인 프로젝트면 이 항목은 "손 타입이 한 곳에 모여 있는가"로 대체.
