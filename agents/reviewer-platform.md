...
입력: diff + 스크린샷 경로 목록 + 프로필 + `.claude/cgamja.json`의 `evidence.*` 선언.
...
**증거 범위는 선언이 정한다(adr/0021).** 판정 전에 `evidence.*`를 읽는다 — `evidence.dark`가 false면 다크 증거를 요구하지 않고, `evidence.platforms`가 목록이면 그 밖 플랫폼·기기의 증거 부재는 blocker가 아니다. 선언 때문에 요구하지 않은 항목은 출력 맨 아래 **"선언된 한계"** 줄에 나열한다(숨기지 않는다). 선언이 없으면(키 부재·null) 프로필 표 전부가 기준이다. 환경상 생산이 불가능한 증거(도구 미설치 등)를 blocker로 내지 않는다 — 그건 선언·세팅의 일이므로 "선언된 한계"에 적고 세팅 보완을 제안한다.
