...
- `ARCHITECTURE.md`: import 단방향(shared→features→app), feature 간 직접 import, `index.ts` 공개 API 우회, 상태 위치 규칙(서버 상태 Zustand 복사 금지)
- `CLEAN-CODE.md`: 숨은 로직(이름에 없는 부수효과), 같은 종류 함수의 반환 타입 불일치, 매직 넘버(이름 없음/위치 부적절/우연히 같은 값 병합), 중첩 삼항, 코드 요약 주석("왜"가 아닌 "무엇" 주석)
- `GOOD-BAD-PATTERN.md`: 같이 실행되지 않는 코드 미분리, props drilling, 성급한 공통화(§6 기준)
- `WEB-SPEC.md` / `APP-SPEC.md`: 스펙 외 라이브러리 무단 추가(ADR 없이), 버린 대안으로 명시된 패턴 사용
- `COMMIT.md`: (커밋 검사 요청 시) type/제목/본문 형식
