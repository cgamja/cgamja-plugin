...
입력: diff + `.claude/cgamja.json`(`a11y.runtime`, `platform.profile`) + (있으면) 런타임 검사 결과·키보드 Tab 시퀀스 증거 경로 + `.claude/rules/platform.md`.

절차:
1. 증거 확인: `a11y.runtime`이 선언돼 있으면 런타임 결과 `violations: 0 (serious+)` 기록과 Tab 시퀀스 목록이 있는가 — 없으면 blocker("증거 없음"), 코드가 맞아 보여도. `a11y.runtime`이 null이거나 `platform.profile: native`면 런타임 결과 대신 **§2 수동 체크 표**(`references/a11y-frontend.md` §3)가 PR에 있는가로 판정하고, 없으면 blocker.
...
7. 네이티브(`platform.profile: native`, diff가 네이티브 컴포넌트를 건드릴 때만): 접근성 role/label/state 속성, 터치 타깃 44pt, 폰트 스케일링 끈 곳. 런타임 도구가 없으므로 1번의 수동 표가 증거다.

**증거 범위는 선언이 정한다(adr/0021).** `a11y.runtime`·`evidence.*` 선언 안의 증거 부재만 blocker다. 선언 밖(예: `evidence.dark: false`인 프로젝트의 다크 대비, `evidence.platforms`에 없는 플랫폼의 수동 표)의 부재는 blocker가 아니라 출력 맨 아래 **"선언된 한계"** 줄에 나열한다. 환경상 생산 불가능한 증거를 blocker로 내지 않는다 — "선언된 한계"에 적고 세팅 보완을 제안한다.
