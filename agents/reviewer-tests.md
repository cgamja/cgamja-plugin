...
3. **mock** — 모듈 mock(`vi.mock`·`jest.mock` 등 러너의 모듈 대체) 목록: 대상이 네트워크 경계(`mock.boundary`)인가, 아니면 자식 컴포넌트·훅·store·프레임워크 내부인가. 후자는 should(3개 넘으면 blocker — 컴포넌트 분리 신호). 브라우저 계층에서 모듈 mock과 경계 mock 혼용은 blocker.
...
출력: `references/review-lenses-frontend.md` §4 형식. mock 수·대상은 지적이 없어도 한 줄 요약한다("mock 2개: 경계 핸들러만").
