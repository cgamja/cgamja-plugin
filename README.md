...
```
skills/
  develop-setup/   프로젝트를 읽어(스택 불문·brownfield) 철학의 강제 수단을 대조·최소 제안·프로브하고 `.claude/cgamja.json`에 선언
  develop-fe/      오케스트레이터 — 티어 → 스펙 → 구현 루프 → 증거 → 리뷰 → PR. test-fe/review-fe를 호출
  test-fe/         테스트 작성 — 계층 선택(선언 tests.layers), 역할 쿼리·경계 mock, red 게이트(사람 승인), test() 커밋
  review-fe/       리뷰 — 티어별 렌즈 L1~L7을 persona 서브에이전트로(Tier-3/PR만 ce-code-review 병행), PR 모드
references/        관심사별 지식 — 앞부분 스택 무관 원칙(절차가 참조) / 뒷부분 "검증된 구현"(스택·날짜, 세팅이 참조). adr/0014
...
bin/               openspec 래퍼
```
스킬 이름의 `-fe`는 프론트엔드 관심사(디자인 원천·접근성·플랫폼·스크린샷 증거)가 절차에 있다는 뜻이지 특정 스택이 아니다(adr/0014 §6). 절차(`skills/`·`agents/`)에는 프레임워크·도구 이름이 조건으로 나오지 않는다 — `tests/test_stack_words.sh`가 검사한다. 프로젝트의 사실은 대상 레포의 `.claude/cgamja.json`(선언)에.

## 스킬

### `develop-setup`
기존/새 프론트 프로젝트(스택 불문, brownfield 포함)를 develop-fe가 작동하는 상태로. 스택을 묻거나 바꾸거나 스캐폴드하지 않는다: `scripts/preflight.sh`가 매니페스트·러너·린터·테스트 규약·계약 후보·타 도구 채널(OpenSpec·CE·CLAUDE.md/AGENTS.md)을 **발견**하고 철학 P1~P10의 강제 수단 **대조표**를 낸다 → ✗만 기존 도구 위에 최소로 제안(검증된 조각은 `references/*` 구현 절·`templates/react/`) → `scripts/smoke.sh check`로 "진짜 막나" 프로브 → `.claude/cgamja.json`·CLAUDE.md·rules·훅 기록. 사용자 콘텐츠는 섞여 있는 오픈소스의 채널 그대로(adr/0014 채널 맵).
...
- UI 증거는 `.claude/rules/platform.md` 프로필(web-desktop / web-mobile / native)대로 — 뷰포트·다크·키보드 + 접근성 런타임 + Tab 시퀀스
...
### `test-fe`
시나리오(OpenSpec `#### Scenario` 또는 문장) → 계층 하나(unit / browser / e2e — 선언 `tests.layers`) → 역할 쿼리·경계 mock → **실패 출력 + 이유**를 보여 사람 승인(red 게이트, adr/0009) → `test(scope):` 커밋. 구현은 하지 않는다. 단독: "커버 안 된 시나리오 찾기", "flaky 원인 분류". assertion 완화·skip로 초록 만들기는 거부. 기준 `references/tdd-frontend.md`.
