...
| 구현·테스트 작성 unit(`opsx:apply`의 기본 경로 — unit packet 서브에이전트, adr/0027), 스냅샷 `summary.md` 작성 | **`sonnet`** | 스펙·테스트가 정해진 뒤의 실행. 초록/빨강이 오라클 |
| 스크린샷 촬영·비교 나열, computed style 수집, 린트 결과 정리 | **`haiku`** | 기계적 수집 |
| 코드 리뷰 에이전트(`reviewer-cgamja`, baby의 `reviewer-correctness`) | **`opus`** 기본(baby는 `sonnet` 하향, adr/0020) | 리뷰어는 작성자보다 약하면 안 된다 |
| `/code-review`, `/ce-commit*` 등 Skill 호출 | 세션 모델 (지정 불가) | Skill 도구는 모델 인자 없음 — 내부에서 자체 서브에이전트를 띄우기도 함 |
...
- **Workflow**: `agent(prompt, {model: 'sonnet'})`. 역할별로 다르게.
...
- `agents/*.md` persona 파일의 frontmatter `model:`이 기본값이다(렌즈 체계는 adr/0026으로 폐지, git 이력 참조).

## 3. 하지 않는 것
- 본체를 haiku/sonnet으로 띄우고 "중요하면 올리기" — 티어 판정을 약한 모델이 하면 낮은 티어로 새는 쪽으로 틀린다(관찰: 작업량으로 판단해 버림).
- 리뷰 에이전트를 비용 때문에 haiku로 — 리뷰가 작성자(sonnet)보다 약하면 게이트가 아니다.
...
- reviewer-cgamja(opus)의 지적이 3회 연속 전부 code-review와 중복 → 철학 축의 검사 항목이 좁은 것 — 검사 문서(docs/spec) 범위 점검
