...
**원칙**: 접근성은 에이전트가 "신경 써서" 되는 게 아니라 **린트 → 런타임 검사 → 역할 쿼리 테스트** 세 층이 막아야 한다. 프로젝트는 `.claude/cgamja.json` `a11y.lint`·`a11y.runtime`에 1·2층 도구를 선언하고(없으면 `null` — 세팅 대조표의 제안 항목), 이 문서는 세 층이 못 잡는 것(의미·순서·모바일)의 체크 기준이다. 리뷰(review-cgamja·code-review)와 UI task 증거(`develop-fe/workflow.md` 2장)가 이 파일을 참조한다. §5는 검증된 구현.

## 1. 세 층
| 층 | 선언 키 | 언제 | 잡는 것 | 없을 때(`null`) |
|---|---|---|---|---|
| 린트 | `a11y.lint` | PostToolUse, 밀리초 | 이미지 대체 텍스트 없음, 클릭만 있는 비대화형 요소, 역할 없는 터치 요소, 라벨 없는 input | 리뷰 2축(review-cgamja·code-review)이 §2를 diff에서 본다 |
| 런타임 | `a11y.runtime` | UI task 끝, `commands.verify` 또는 E2E | 대비, 이름 없는 버튼, 중첩 인터랙티브, ARIA 오용, 헤딩 순서 | 수동 표(§3) + 키보드 1회 |
| 테스트 | (workflow 3-2 쿼리 규칙: role/label > 텍스트 > testID) | 모든 컴포넌트 테스트 | 역할로 못 찾으면 = 보조기기도 못 찾음 | — |
...
| 증상 | 대응 |
|---|---|
| 접근성 라벨을 모든 요소에 붙임(과잉) | 린트의 중복 role 규칙 + 리뷰: "보이는 텍스트가 있으면 별도 라벨 금지" |
| 컨테이너에 role="button"으로 디자인 맞춤 | 네이티브 요소 + 스타일 reset. 리뷰 거부 사유 |
| 테스트를 test-id로 통과시킴 | workflow 3-2 쿼리 순서 + 린트(쿼리 우선순위 규칙) |
