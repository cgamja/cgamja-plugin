...
프론트엔드 작업을 받아서 PR까지 가는 절차. 근거는 `references/methodologies.md`, 도구 설정은 `references/openspec-setup.md`, 결정 기록은 `adr/`.
...
2. 체크 정하기: 기존 테스트 수정/추가 **또는** 스크린샷 1장. 순수 스타일이면 스크린샷만
...
4. **`/opsx:apply`** — task 하나씩:
   - 테스트 task: Edit 시 권한 프롬프트가 뜬다 — **사람이 diff를 승인하는 것이 red 게이트**(adr/0009). 승인되면 실행 → **실패 출력과 이유("기능 미구현", import 오류 아님)를 보여주고** `test(scope):` 커밋. 거부되거나 비대화형이라 승인자가 없으면 **구현 task로 넘어가지 말고 멈춰서** 사람에게 알린다(테스트 없는 구현 금지)
   - 구현 task: 테스트 파일은 건드리지 않는다(Edit는 ask, Bash 쓰기는 deny). 초록 + PASS_TO_PASS → `[x]` → `feat(scope):` 커밋. diff에 `*.test.*`가 섞이면 안 됨
   - UI task: `agent-browser`(`[TODO]`)로 **375/768/1280** 스크린샷, 콘솔 에러 0. 스냅샷이 있으면 `reference@2x.png`와 나란히 비교해 차이 나열 — 값은 스크린샷이 아니라 `getComputedStyle`로 확인(`figma-design-source.md` §6)
...
5. **`/ce-code-review`** (새 컨텍스트) — **Skill 도구로 `compound-engineering:ce-code-review`를 부른다. Agent 도구로 리뷰 서브에이전트를 즉석 제작하지 않는다**(2026-08-21 두 번 연속 이탈 — ce-code-review의 `docs/solutions` 조회·보안·테스트 검사가 빠진다). **반드시 스펙 경로를 넘긴다**: `plan:openspec/changes/<slug>/specs/<cap>/spec.md` + "각 `### Requirement:`/`#### Scenario:`를 요구사항으로 취급해 Requirements Completeness를 작성하라". 안 넘기면 ce-code-review는 OpenSpec 포맷을 못 읽어 스펙 대비 누락 검사가 조용히 빠진다(adr/0001). 지시: "스타일 말고 spec 대비 빠진 것·정확성만, mock 수·대상 보고, `feat` diff에 테스트 파일 없음 확인". 반영 후 재실행 1회까지. 그다음 `/ce-simplify-code` 1회
...
6. 전부 archive 후 통합: e2e 스모크 + 주요 플로우 스크린샷 + `/ce-code-review` 전체 diff
...
### 3-2. 테스트 규칙 (상세: `references/tdd-frontend.md` — 시나리오→계층 결정표와 루프)
- TDD의 목적은 품질이 아니라 **리뷰 게이트 + 변조 방지**(adr/0004). 인터랙션·레이아웃은 Vitest Browser Mode(`*.browser.test.tsx`), 로직·훅은 jsdom(`*.test.ts(x)`), E2E 3~5개
- 역할·라벨로 쿼리, `userEvent`, mock은 **네트워크 경계(MSW)에서만**. 자식 컴포넌트·React 내부 mock 금지
- 테스트 파일 수정은 테스트가 작업 대상일 때만. assertion 완화·skip·tolerance·snapshot 재생성으로 초록 만들기 금지. 못 만들면 **실패한 assertion 원문**과 함께 멈춘다
- 기존 초록은 초록 유지(PASS_TO_PASS)
- 안 쓰는 테스트: 렌더 횟수, 구현 세부, 마크업 snapshot
