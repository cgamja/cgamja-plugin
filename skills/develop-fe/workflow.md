...
4. **`/opsx:apply`** — task 하나씩:
   ...
   - UI task: 뷰포트·다크모드·키보드 증거는 **`.claude/rules/platform.md` 프로필**대로(`references/platform-fit-frontend.md`; 없으면 웹 375/768/1280) 선언된 스크린샷 수단(`evidence.screenshot`; null이면 촬영 수단을 탐색해 **직접 촬영을 먼저 시도**하고, 안 되면 사용자에게 요청 — 탐색 순서는 `references/evidence-capture.md`(adr/0021))으로 찍고, 콘솔 에러 0. 접근성 증거: axe `serious+` 0건 + Tab 시퀀스 목록(`references/a11y-frontend.md` §3). 스냅샷이 있으면 `reference@2x.png`와 나란히 비교해 차이 나열 — 값은 스크린샷이 아니라 `getComputedStyle`로 확인(`figma-design-source.md` §6)
