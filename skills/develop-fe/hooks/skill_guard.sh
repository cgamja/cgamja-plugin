...
#  - ce-code-review           → args에 plan: 없으면 deny (OpenSpec 포맷을 못 읽어 요구사항 누락 검사가 조용히 빠짐). v2에서는 리뷰 2축(review-cgamja + /code-review)이 기본(adr/0026)
...
case "$name" in
  ...
  ce-code-review)
    grep -q 'plan:' <<<"$args" || deny "[develop-fe] /ce-code-review 는 스펙 경로 없이 부르면 OpenSpec 포맷을 못 읽어 Requirements Completeness 검사가 조용히 빠집니다(adr/0001). \`plan:openspec/changes/<slug>/specs/<cap>/spec.md\` 와 힌트('각 ### Requirement:/#### Scenario:를 요구사항으로 취급')를 args에 넣어 다시 호출하거나, 리뷰 2축(cgamja:review-cgamja + /code-review, adr/0026)을 쓰세요.";;
esac
exit 0
