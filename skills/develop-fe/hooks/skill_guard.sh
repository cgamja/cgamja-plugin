...
#  - ce-code-review           → args에 plan: 없으면 deny (OpenSpec 포맷을 못 읽어 요구사항 누락 검사가 조용히 빠짐). review-fe 스킬이 경로를 조립해 부른다(adr/0012)
...
case "$name" in test-fe|review-fe) touch "$STATE/$name";; esac   # test_nudge.sh가 읽는 마커(adr/0010 결함 2)
case "$name" in
  ce-plan|lfg)
  ...
  ce-code-review)
    grep -q 'plan:' <<<"$args" || deny "[develop-fe] /ce-code-review 는 스펙 경로 없이 부르면 OpenSpec 포맷을 못 읽어 Requirements Completeness 검사가 조용히 빠집니다(adr/0001). \`plan:openspec/changes/<slug>/specs/<cap>/spec.md\` 와 힌트('각 ### Requirement:/#### Scenario:를 요구사항으로 취급')를 args에 넣어 다시 호출하거나, /review-fe 스킬을 쓰세요(경로를 조립해 줍니다).";;
esac
exit 0
