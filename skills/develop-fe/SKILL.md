...
프론트엔드 작업을 받아서 PR까지 가는 방법. 절차 전체는 `workflow.md`에 있고, 이 파일은 **언제 무엇을 읽을지**만 정한다.
...
- `/ce-code-review`를 스펙 경로 없이 호출 — OpenSpec 포맷을 못 읽어 요구사항 누락 검사가 조용히 빠진다. 항상 `plan:<changeRoot>/specs/<cap>/spec.md` + 힌트.
...
| `references/tdd-frontend.md` | 테스트 task를 쓸 때 — 시나리오→계층 결정표, 빨강·초록 루프, 쿼리/mock 규칙 |
...
| `adr/` | 절차·도구를 바꾸려 할 때 — 먼저 ADR, 그다음 workflow.md |
| `hooks/` | 스킬 frontmatter 훅 스크립트(Skill 가드, 세팅 점검). 규칙을 추가·완화할 때 — adr/0007 |
