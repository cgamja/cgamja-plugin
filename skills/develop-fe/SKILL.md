...
- **UI 검증**: `cgamja:browser-testing-with-devtools`(콘솔 0·DOM·네트워크) → 스크린샷 증거(`references/evidence-capture.md`).
...
- Agent 도구로 리뷰어·테스트 작성자를 즉석 제작 — `review-cgamja`/`code-review`/`test-driven-development`를 쓴다. 서브에이전트는 `model:` 명시(`references/model-routing.md`).
...
| `references/openspec-setup.md` | OpenSpec 세팅·`/opsx:*` 이상 동작 시 |
| `references/figma-design-source.md` | 화면 task 시작 전(§3), 스냅샷(§2), 디자인 갭(§5) |
| `references/api-contract.md` | API task — 계약 판정·DRAFT 스텁·retrofit |
| `references/evidence-capture.md` | UI 증거 촬영 전 — 수단 탐색 순서(adr/0021·0025) |
| `references/a11y-frontend.md` | UI task 접근성 체크·증거 |
| `references/platform-fit-frontend.md` | 플랫폼 프로필 기준 |
| `references/model-routing.md` | 서브에이전트 띄울 때마다 |
| `references/project-conventions.md` | 규칙 배치표·아키텍처 경계 |
