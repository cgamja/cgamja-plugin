...
- 테스트를 초록으로 만들기 위한 테스트 수정 — red 게이트는 **세션당 1회 사람 승인**(첫 테스트 Edit의 권한 프롬프트, adr/0018), 이후엔 각 red의 **실패 출력 원문을 본문으로 보고**하고 `test():` 커밋으로 분리한다. 쉘로 쓰기(`sed -i`/`perl -pi`/리다이렉트)와 인라인 `TDD_PHASE=`는 거부된다. 첫 승인이 없으면 구현으로 넘어가지 말고 멈춰라. 못 고치면 실패한 assertion 원문과 함께 멈춰라
