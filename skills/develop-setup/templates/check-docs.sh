...
# 1) 백틱 안의 경로 (src/, docs/, design/, openspec/, .claude/, test/, e2e/ 로 시작, 글롭·플레이스홀더 제외)
for f in "${files[@]}"; do
  grep -oE '`(src|docs|design|openspec|\.claude|test|e2e|scripts)/[^`*<>{}]+`' "$f" 2>/dev/null | tr -d '`' | sort -u | while read -r p; do
    p="${p%/}"
...
for f in "${files[@]}"; do
  grep -oE '`(pnpm|npm run|yarn|bun) [a-z:-]+`' "$f" 2>/dev/null | awk '{print $2}' | tr -d '`' | sort -u | while read -r s; do
