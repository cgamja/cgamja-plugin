...
4. **red** — 껍데기(빈 export)로 import만 통과시키고 실행. **실패 출력 원문(assertion별 핵심 줄) + 이유 한 줄**("요소 없음 — 기능 미구현")을 **사용자에게 보이는 본문에** 붙인다 — thinking·요약만으로는 게이트가 성립하지 않는다(2026-08-25 되물음 2회 실측). import 에러·오타는 red가 아니다 → 3으로.
5. **사람 게이트** — 이 세션의 **첫** 테스트 파일 Edit에만 권한 `ask`가 뜬다(세션당 1회, adr/0018 — 이후 편집은 묻지 않는다). 게이트의 신호는 4의 실패 원문 보고 + `test(scope): <시나리오>` 커밋(`/ce-commit`)이다. 첫 ask가 거부되거나 비대화형이라 승인자가 없으면 **구현으로 넘어가지 말고 멈춰서** 알린다.
