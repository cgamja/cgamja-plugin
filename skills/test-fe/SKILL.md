---
name: test-fe
description: 프론트엔드 테스트 코드를 쓰는 기준과 루프 — 시나리오(OpenSpec `#### Scenario` 또는 사용자 문장)를 받아 계층(jsdom / Vitest Browser Mode / E2E) 하나를 고르고, 역할 쿼리·MSW 경계 mock으로 실패하는 테스트를 먼저 쓰고, 실패 출력과 이유를 보여 사람 승인(red 게이트)을 받아 `test(scope):` 커밋까지 간다. 사용자가 "테스트 써줘", "이 컴포넌트 테스트 추가", "이 시나리오 테스트로", "테스트 커버 안 된 거 찾아줘", "이 테스트 왜 flaky해"라고 하거나 /test-fe 를 부르거나, develop-fe의 테스트 task를 수행할 때 반드시 사용한다. Expo(jest-expo+RNTL)도 같은 루프. 테스트를 초록으로 만들기 위해 assertion을 고치는 요청은 거부하고 이 스킬의 규칙을 설명한다.
---
...
## 절차
1. **계층 하나 고르기** — `tdd-frontend.md` §1 표. 인터랙션·포커스·레이아웃이면 Browser Mode(`*.browser.test.tsx`), 로직·훅·텍스트만이면 jsdom, 화면을 건너면 E2E(3~5개 고정 목록에 있을 때만). 고른 이유를 한 줄로.
2. **같은 걸 검증하는 테스트가 이미 있나** grep(시나리오 키워드, 컴포넌트명). 있으면 그 파일에 추가하고 중복 테스트를 만들지 않는다.
3. **테스트 작성** — §3 규칙: `getByRole` > `getByLabelText` > … > `getByTestId`(최후, 이유 주석 필수). mock은 MSW 경계에서만, 핸들러는 orval 생성물(`get<Title>Mock`)을 기본으로 시나리오별 `server.use`. 자식 컴포넌트·훅·store mock 금지. 비동기는 자동 재시도 expect. 테스트 이름은 `WHEN … THEN …` 원문.
...
- "flaky 원인": `tdd-frontend.md` §1 CI 주의(Browser Mode 메모리·iframe), §3(`waitForTimeout`, `setTimeout`, 공유 상태), §4 표로 분류. **끄거나 retry를 늘리는 것으로 끝내지 않는다** — 원인 분류 결과와 고친 assertion을 보여준다. 2번 고쳐도 flaky면 그 테스트만 jsdom으로 내린다(삭제 아님).
...
- mock 목록("MSW 핸들러 2개, 기타 0")
- 이것 없이 "테스트 작성했습니다"는 완료가 아니다.

## 절대 하지 않는 것
- 테스트와 구현을 한 턴에
- `it.skip`/`.only`/`expect(true)`/`toBeDefined`만 있는 assertion/`vi.mock("src/**")`(api 제외)
...
| `references/api-contract.md` §4 | MSW 핸들러를 생성물에서 가져올 때 |
