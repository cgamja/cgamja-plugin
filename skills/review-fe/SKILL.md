...
3. **증거 수집(`haiku` Explore 1개)**: 스크린샷 경로 존재 확인, 접근성 런타임(`a11y.runtime`) 결과, `commands.verify` 마지막 실행 출력(`.claude/state/verify.last.log` — Stop 훅이 성공·실패 모두 저장), 테스트 커밋/feat 커밋 분리 목록, 스펙 시나리오 목록. 이 묶음을 모든 persona에 같은 텍스트로 넘긴다.
